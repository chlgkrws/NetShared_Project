			...
			if (isMember.equals(memberOk)) {
				...
				if (isPossibleRegister) {
					...
					if (canIJoinYourMatching != null) {
						...
						String leaderId = canIJoinYourMatching.getLeader_id();
					...
					// 매칭테이블에 남는자리가 없다면 wait_tbl에 파티장의 대기가 있는지 확인한다.
						...
						setMatchingInfo(matchingVO, userId, 0, headCount, leaderVO.getMaxNumberOfMember() == 2 ? true : false, true, 0, netId, netPassword);
